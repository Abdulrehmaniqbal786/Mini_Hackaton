 ...
 const change_handle = (e) => {
  ...
  const submit_handle = (e) => {
    ...
    // read_user_data(data.name, data.bloodGroup,data.country,data.city)
      ...
      write_user_data(`${myJSON}/Accepter`, data).then((res) => {
        dispatch(set_user_auth({ data1:data }));
        localStorage.setItem("Procurer",true)
      }).catch((error) => {
      console.log(error);
    });
  ...
  const itemms = localStorage.getItem("Procurer")
  return (
    <div>
    {itemms?<>
  ...
  <div className="card-body">
    <h2 className="card-title">Accept Blood information</h2>
    <p>Name:{data.name}</p>
      ...
      <div>
        ...
        <div className='flex justify-center items-center '>
          ...
          <div className="border-4 rounded border-black w-6/12 relative flex flex-col p-4 rounded-md text-black bg-white">
            ...
            <div className="text-sm font-normal mb-4 text-center text-[#1e0e4b]"><h1 className='text-2xl font-bold'>Accept Blood</h1></div>
